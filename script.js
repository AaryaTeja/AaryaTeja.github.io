(function () {
  ...
  function init() {
    ...
    const main = $('ap-main');
    let spacer = null, current = 0, target = 0, vel = 0;
    ...
    all('a[href^="#"]').forEach(a => {
      a.addEventListener('click', (e) => {
        ...
        if (!tgt) return;
        e.preventDefault();
        const y = tgt.getBoundingClientRect().top + (spacer ? current : window.scrollY);
        window.scrollTo({ top: y - 20, behavior: spacer ? 'auto' : 'smooth' });
      });
    });
  }
